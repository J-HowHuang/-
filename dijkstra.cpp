...
double threatSofar(int** route, int* x, int* y, int* r, int* p, int m, int w, int t);
...
//	cout << threatof(originRoute, x, y, r, p, m, w, 0) << "\n";
		...
		for(int i = -unit; i <= unit; i += unit)
			for(int j = -unit; j <= unit; j += unit){
				...
				if(currentX + i > n || currentY + j > n)
					continue;
				if((currentX + i - startX) * (endX - startX) < 0 || (currentY + j - startY) * (endY - endY) < 0)
					continue;
				...
				//	source[currentX + i][currentY + j][1] = currentY;
					...
					tempRoute[1][0] = endX;
					tempRoute[1][1] = endY;
					tempRoute[2][0] = currentX + i;
					tempRoute[2][1] = currentY + j;
					while(nextsourceX != -1){
					...
					for(int k = 0; k < turnCnt; k++){
						double threatofRoute = threatSofar(route, x, y, r, p, m, w, k);
					//	cout << i << " " << j << " if ignore " << k << " corner " << threatofRoute << "\n";
    ...
    {
        bool cor = turnOrNot(route[i-1][0], route[i-1][1], route[i][0], route[i][1], route[i+1][0], route[i+1][1]);
        if(cor)
        {
            correctturnCnt++;
            route1[i] = 1;
...
double threatSofar(int** route, int* x, int* y, int* r, int* p, int m, int w, int t){
    ...
    for(int i = 1; i <= ( route[0][0] + 1 ) ; i++){
        ...
        if( ( i == route[0][0] - t + 1) )
        {
            if(i > 1 && route[0][0] > 0)
            {
                bool corTemp = turnOrNot(route[i - 1][0], route[i - 1][1], route[i][0], route[i][1], route[route[0][0] + 2][0], route[route[0][0] + 2][1]);
                corner += corTemp;
            }
            len = length(route[i][0], route[i][1], route[route[0][0] + 2][0], route[route[0][0] + 2][1]);
            cmpntX = (route[route[0][0]+ 2][0] - route[i][0]);//culculate x component
            cmpntY = (route[route[0][0]+ 2][1] - route[i][1]);
        }
        else
        {
            if(i > 1 && route[0][0] > 0)
